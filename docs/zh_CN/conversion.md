...
OneBot11协议中未定义文件消息段也没有上传消息的动作，所以默认不支持转换，如何转换需要取决于OneBot11实现并提供转换方式
...
| 无                      | `version`     | 无   | OB12: resp[`[get_version]`](#get_version_info-get_version) |
...
OneBot 11协议中并无获取文件的动作，因此由实现根据缓存自行决定
